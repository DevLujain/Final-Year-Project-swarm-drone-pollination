...
class FieldHealthMapper:
    ...
    def _reload_log(self):
        ...
        with open(LOG_CSV, newline='') as f:
            for row in csv.DictReader(f):
                ...
                if status == 'POLLINATED' and fid and fid not in self.pollinated:
                    self.pollinated[fid] = run_id
                    new += 1
        if new:
    ...
    def add_live_event(self, flower_id: str, run_id: str, drone_id: int):
        ...
        if flower_id and flower_id not in self.pollinated:
            self.pollinated[flower_id] = run_id
            total   = len(self.flowers)
            polled  = len(self.pollinated)
